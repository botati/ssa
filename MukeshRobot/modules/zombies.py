...
async def rm_deletedacc(show):
    ...
    del_u = 0
    del_status = "تنظيف المجموعة ،  تم العثور على  0  حسابات محذوفة."
    if con != "clean":
        kontol = await show.reply("`البحث عن الحساب المحذوف`")
    ...
    if not admin and not creator:
        return await show.reply("عذرًا، أنت لست مشرفًا ‼️")
    memek = await show.reply("`الحسابات المحذوفة...`")
    del_u = 0
    del_a = 0
